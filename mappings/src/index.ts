 ...
 * matching on `name` (exact or normalized) is not — a renamed component set
 * can silently stop matching by id, which is exactly the failure this field
 * makes visible.
 */
export interface ComponentMapLookup {
  entry: ComponentMapEntry | null;
  matchedBy: "figmaNodeId" | "name" | "normalizedName" | null;
 ...
 * into raw geometry trees. Node id is now the primary key; an exact name
 * match is the next fallback (entries may legitimately have
 * `figmaNodeId: null`), and a normalized (case/whitespace/singular-plural
 * insensitive) name match is the last resort — see
 * `normalizeComponentSetName`. A name/id disagreement is reported rather
 * than ignored.
 */
export function lookupComponentMapEntry(
  ...
  if (byName) return { entry: byName, matchedBy: "name" };

  const normalizedTarget = normalizeComponentSetName(figmaComponentSetName);
  const byNormalizedName = componentMap.entries.find(
    (entry) => normalizeComponentSetName(entry.figmaComponentSet) === normalizedTarget,
  );
  return byNormalizedName
    ? { entry: byNormalizedName, matchedBy: "normalizedName" }
    : { entry: null, matchedBy: null };
}

/** Looks up a component-map entry by its (exact or normalized) Figma component set name. */
