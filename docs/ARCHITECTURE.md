...
4. **`cli/`** — `--input`, `--output`, `--package`, `--prefix`,
   ...
   `primitives` объявляет `iOS`, а `typography` — `ios`), поэтому
   `--exclude-mode ios` убирает оба. Каталог `--output` не просто
   пополняется, а синхронизируется: файл, который генератор писал раньше, но
   в этом запуске больше не производит (переименованная/удалённая в Figma
   коллекция или ветка, отфильтрованный режим, прошлый `--layout`),
